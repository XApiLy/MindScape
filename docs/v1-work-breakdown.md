...
## 15. M2 追加任务：本地知识、聚焦连续性与通用导入

M2 使用以下增量编号。它们建立在 Frozen V1 的会话图、运行事件和导入 RC1 之上，不重写已有任务语义。平台专用导入、快速/详细分析和漫游分析仍使用本文件原有 `IMP-008~010`、`AIX-004~014`，在 M3 执行。

### 15.1 ARC-M2：跨团队契约

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| ARC-M2-001 | Effective Run Profile | Provider、Model、reasoning、生成参数、上下文、工具、预算和来源可冻结回放 | ARC-005, PROV-003 | M |
| ARC-M2-002 | FocusFrame 契约 | 当前目标、父节点、包含/排除引用、记忆版本和作用域 | ARC-004, CORE-008 | M |
| ARC-M2-003 | KnowledgeEntity / Relation | Goal、Decision、Constraint、Question、Source、Project、Topic 和显式关系 | ARC-001, ARC-010 | L |
| ARC-M2-004 | Markdown Projection | 稳定 ID、frontmatter、EvidenceRef、Wikilink 和修订边界 | ARC-M2-003, DATA-009 | M |
| ARC-M2-005 | 通用导入语义 | ImportSource、ImportedMessage、ParseReport、RawTrack 进入会话图的规则 | ARC-009, CORE-018 | M |

### 15.2 CORE-M2：焦点、分支与知识编译

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| CORE-M2-001 | FocusFrame 生命周期 | 创建、切换、关闭、恢复和版本化 | ARC-M2-002, DATA-M2-001 | M |
| CORE-M2-002 | 分支记忆作用域 | `inherit / local / exclude / promote` 和主线/任务分支隔离 | ARC-M2-002, CORE-005~007 | L |
| CORE-M2-003 | 知识候选与确认 | candidate/inferred/confirmed/rejected/superseded/stale 状态和回流候选 | ARC-M2-003, ARC-M2-005 | L |
| CORE-M2-004 | 混合检索上下文编译 | FTS、向量、关系邻居、权限、预算和排除项进入 ContextSnapshot | CORE-010~013, RAG-M2-001~004 | L |

### 15.3 DATA-M2：本地知识、日志与通用导入

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| DATA-M2-001 | FocusFrame 与作用域存储 | 事务保存、查询、版本和启动恢复 | DATA-003, ARC-M2-002 | M |
| DATA-M2-002 | 实体关系证据表 | 实体、关系、EvidenceRef、状态、版本和软删除 | DATA-003, ARC-M2-003 | L |
| DATA-M2-003 | 讨论日志账本 | 原始事件、可读日志和分支日志的来源关系 | DATA-008, ARC-M2-004 | M |
| DATA-M2-004 | Markdown Vault 投影 | 标准目录、安全路径、frontmatter、稳定 ID 和重建 | DATA-002, ARC-M2-004 | L |
| DATA-M2-005 | Markdown 修订回读 | 用户编辑产生修订事件，重新索引，不覆盖旧版本 | DATA-M2-004, ARC-011 | M |
| DATA-M2-006 | 通用导入来源存储 | 原文件、指纹、ImportRevision、ImportedMessage、ParseReport | DATA-009~010, ARC-M2-005 | L |
| DATA-M2-007 | 通用导入恢复 | 重复、损坏、超大、取消、中断和重启恢复 | DATA-014~015, DATA-M2-006 | M |
| DATA-M2-008 | 删除与索引失效 | 删除/否决/过期实体和来源后旧检索记录失效 | DATA-016, DATA-M2-002 | M |
| DATA-M2-009 | 通用 Markdown / JSONL / TXT 适配 | 真实正常、异常和退化样本可解析并可预览 | IMP-004, IMP-006~007 | L |
| DATA-M2-010 | 本地索引重建 | 从 SQLite、Markdown 和原文重建全文/向量索引 | DATA-M2-004~009 | L |

### 15.4 CAN-M2：画布与来源投影

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| CAN-M2-001 | 主线/任务分支标识 | 显示分支类型、来源和当前 FocusFrame | CORE-M2-001~002, CAN-005 | M |
| CAN-M2-002 | 创建任务 FocusFrame | 从节点创建独立任务分支并明确继承范围 | CORE-M2-002, CAN-006 | M |
| CAN-M2-003 | 分支切换与返回 | 主线、探索、任务和复盘分支切换后视图与语义一致 | CORE-M2-001, DATA-M2-001 | M |
| CAN-M2-004 | 导入来源入口 | 导入节点与原文轨来源清晰，不伪装成本地生成 | IMP-M2-005, CORE-018 | M |
| CAN-M2-005 | 知识与日志引用投影 | 显示实体、Markdown、来源和排除范围入口 | ARC-M2-004, CORE-M2-004 | M |
| CAN-M2-006 | 回流候选界面 | 查看任务分支成果，提交或拒绝回流 | CORE-M2-003 | M |
| CAN-M2-007 | 200 节点分支基准 | 分支投影、切换、来源标记和流式不退化 | CAN-019, CAN-M2-001~006 | L |
| CAN-M2-008 | 画布来源集成测试 | 导入、分支、日志、知识投影和重启恢复 | CAN-020, QA-M2-005 | L |

### 15.5 CHAT-M2：运行控制与导入体验

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| CHAT-M2-001 | 运行档案控制入口 | 模型、reasoning、预算、上下文和工具可见可改 | ARC-M2-001, CHAT-003 | M |
| CHAT-M2-002 | 上下文策略选择 | 当前问题、聚焦新问题、分支和原样续接 | ARC-M2-002, CORE-M2-001 | M |
| CHAT-M2-003 | 上下文账单 | 显示引用、排除、Token、来源和预算 | CORE-M2-004, CHAT-011 | M |
| CHAT-M2-004 | 文件导入入口 | 选择、拖放、粘贴、取消和预检 | IMP-002~003, DATA-M2-006 | M |
| CHAT-M2-005 | 导入预览与报告 | 来源、消息、字段、警告、重复和发送范围 | IMP-016~017, DATA-M2-009 | L |
| CHAT-M2-006 | 原样继续 | 导入后不调用生成式分析，创建可继续入口 | IMP-019, CORE-018 | M |
| CHAT-M2-007 | 讨论日志与候选记忆 | 展示日志、候选实体、确认、否决和回流 | CORE-M2-003, DATA-M2-003 | L |
| CHAT-M2-008 | 分支任务体验 | 创建任务分支、查看本地记忆和返回主线 | CORE-M2-002, CAN-M2-002 | M |
| CHAT-M2-009 | reasoning 事件分层 | reasoning 摘要、正文、工具和终态不混淆 | PROV-M2-002, CHAT-005 | M |
| CHAT-M2-010 | M2 Chat 集成测试 | 运行控制、导入、聚焦、分支和知识引用闭环 | CHAT-M2-001~009 | L |

### 15.6 PROV-M2 / RAG-M2：能力与检索

| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| PROV-M2-001 | reasoning 能力声明 | 支持模式、预算、空正文和参数能力可查询 | PROV-001~003 | M |
| PROV-M2-002 | Effective Profile 映射 | OpenAI-compatible / DeepSeek 映射准确且不可静默降级 | ARC-M2-001, PROV-008, PROV-012 | M |
| PROV-M2-003 | reasoning 回归矩阵 | reasoning-only、正文、终态、用量和重启回归 | PROV-M2-001~002, QA-010~012 | L |
| PROV-M2-004 | Embedding 能力边界 | 模型、隐私、费用、维度和版本声明 | ARC-M2-001, ARC-013 | M |
| PROV-M2-005 | VectorIndex 适配器 | 至少一个真实本地向量索引可写、查、删和重建 | PROV-M2-004, DATA-M2-010 | L |
| PROV-M2-006 | 本地检索失败降级 | 向量不可用时全文/关系检索仍可工作并提示 | PROV-M2-005, CORE-M2-004 | M |
| PROV-M2-007 | Provider / Index 脱敏文档 | 不泄漏 Key、私人正文和未授权云端请求 | PROV-017, ARC-013 | S |
| RAG-M2-001 | Markdown 分块索引 | 标题、段落、代码块、实体、分支和来源元数据保留 | DATA-M2-004, DATA-M2-010 | M |
| RAG-M2-002 | FTS 精确召回 | 名称、关键词、中文短语和 ID 可检索 | DATA-M2-002, DATA-M2-010 | M |
| RAG-M2-003 | 向量 + 关系召回 | 语义相似和关系邻居可组合返回证据 | PROV-M2-005, DATA-M2-002 | L |
| RAG-M2-004 | 检索排序与排除 | 作用域、确认状态、权威性、新鲜度、冲突和预算可解释 | CORE-M2-004, RAG-M2-001~003 | L |
...
| ID | 任务 | 交付物与完成标准 | 依赖 | 大小 |
| --- | --- | --- | --- | --- |
| QA-M2-001 | 通用导入样本矩阵 | Markdown、JSONL、TXT 正常、损坏、重复、超大和未知块样本 | DATA-M2-009 | M |
| QA-M2-002 | Markdown Vault 验收 | Obsidian 可打开、frontmatter、链接、日志和重建索引可读 | DATA-M2-004~005 | M |
| QA-M2-003 | 分支记忆污染测试 | 主线/任务分支 inherit、local、exclude、promote 隔离 | CORE-M2-002~003 | L |
| QA-M2-004 | 知识状态与删除测试 | 确认、否决、过期、删除后检索结果正确 | DATA-M2-002, DATA-M2-008 | M |
| QA-M2-005 | M2 纵向闭环 E2E | 讨论/导入、实体确认、FocusFrame、检索、回答、回流候选 | CHAT-M2-010, RAG-M2-004 | L |
| QA-M2-006 | M1 回归与真实 Release | 终态、取消、恢复、Key、脱敏和同一 Build 证据 | QA-010~015, REL-005 | L |
